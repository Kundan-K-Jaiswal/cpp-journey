...
// 3833. Count Dominant Indices   |  Platform : LeetCode
...
// The average of a set of numbers is the value obtained by adding all the numbers together and dividing the sum 
...
int dominantIndices(vector<int>& nums) {
    ...
    for ( int i = sz-2; i>=0; i-- ) { // traversing form the back
        if ( nums[i] * elementCount > sum ) {
            count++;
        }
        sum += nums[i];
        elementCount++;
