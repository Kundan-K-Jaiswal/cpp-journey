...
// 3774. Absolute Difference Between Maximum and Minimum K Elements  |  Platform : LeetCode
...
int absDifference(vector<int>& nums, int k) {
    ...
    int count = 0, smallestSum = 0, largestSum = 0;
    
    while ( count < k ) { 
        smallestSum += nums[count];
        largestSum += nums[nums.size() - 1 - count ];
        count++;
    }

    return abs(smallestSum - largestSum);
