...
// 3838. Weighted Word Mapping  |  Platform : LeetCode
...
string mapWordWeights(vector<string>& words, vector<int>& weights) {
    string ans = ""; // will store our answer 
    for ( string word : words ) {
        int totalWeight = 0; // calculates weight of each word
        for ( char letter : word ) {
            int ind = letter - 'a'; // calculates index for weight calculation
            totalWeight += weights[ind];
        }
        totalWeight %= 26; 
        ans += 'z'-totalWeight; // concates character to our answer variable
    }
    return ans;
}
