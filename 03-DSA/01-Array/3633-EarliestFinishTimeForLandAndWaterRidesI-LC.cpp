...
// 3633. Earliest Finish Time for Land and Water Rides I  |  Platform : LeetCode
...
int earliestFinishTime(vector<int>& landStartTime, vector<int>& landDuration, vector<int>& waterStartTime, vector<int>& waterDuration) {
    int min_duration = INT16_MAX;
    // taking the land ride first
    for ( int i = 0; i<landStartTime.size(); i++ ) {
        for ( int j = 0; j<waterStartTime.size(); j++ ) {
            int duration = landStartTime[i] + landDuration[i];
            if ( waterStartTime[j] > duration ) {
                duration += waterStartTime[j] - duration;
            }
